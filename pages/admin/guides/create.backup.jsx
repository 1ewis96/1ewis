...
import { FileText, Image, List, Plus, Save, ArrowLeft, Eye, Trash2, GripVertical, Code, X, ChevronUp, ChevronDown, Copy, Upload, Loader } from 'lucide-react';
...
function CreateGuidePage() {
  ...
  const uploadImage = async (file, targetField) => {
    ...
    try {
      ...
      return data.url; // Updated to match the response format
    } catch (error) {
  ...
  const handleFileSelect = async (e, targetField) => {
    ...
    const imageUrl = await uploadImage(file, targetField);
    if (imageUrl) {
                                        ...
                                        <div className="space-y-2">
                                          ...
                                          <div className="flex space-x-2">
                                            <button
                                              ...
                                              className="px-2 py-1 bg-gray-800 hover:bg-gray-700 text-white text-xs rounded-lg flex items-center transition-colors"
                                              disabled={uploading}
                                            >
                                              {uploading ? <Loader className="w-3 h-3 mr-1 animate-spin" /> : <Upload className="w-3 h-3 mr-1" />}
                                              {uploading ? 'Uploading...' : 'Upload'}
                                            </button>
                                            <input
                                              type="text"
                                              className="flex-1 px-3 py-2 bg-gray-900/80 border border-gray-700 rounded-lg focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-transparent text-white text-sm"
                                              placeholder="Or enter image URL..."
                                              value={section.image}
